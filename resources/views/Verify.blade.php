...
<head>
    ...
    <style>
        ...
        /* Gaya khusus untuk input OTP agar terlihat renggang */
        .otp-input {
...
<body class="bg-slate-dark flex items-center justify-center min-h-screen p-4 relative overflow-hidden">
    ...
    <main
        ...
        <div
            ...
            <img src="https://images.unsplash.com/photo-1511920170033-f8396924c348?q=80&w=800"
                class="absolute inset-0 w-full h-full object-cover opacity-40" alt="Coffee Steam">
            ...
            <div class="relative z-10 text-center">
                ...
                <h2 class="font-serif text-4xl text-white mb-4 italic uppercase tracking-tighter">Cek Email Anda.</h2>
                <p class="text-gray-400 text-sm font-light italic tracking-wide">
                    "Kami telah mengirimkan kode rahasia untuk memastikan keamanan akun Anda."
                </p>
            </div>
        </div>
        ...
        <div class="w-full md:w-1/2 p-8 sm:p-12 flex flex-col justify-center bg-slate-900/50">
            <div class="mb-10 text-left">
                ...
                <p class="text-gray-400 text-xs uppercase tracking-[0.2em]">Masukkan 6-digit kode yang kami kirimkan</p>
            </div>

            {{-- Pesan Sukses (Jika ada) --}}
            @if (session('success'))
                <div
                    class="mb-6 p-4 rounded-xl bg-green-500/10 border border-green-500/20 text-green-500 text-xs italic">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form action="{{ route('register.verify.post') }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label
                        class="block text-[10px] font-bold uppercase tracking-widest text-yellow-zocco mb-2 ml-1">Kode
                        Verifikasi</label>
                    <div class="relative">
                        ...
                        <input type="text" name="otp" placeholder="000000" maxlength="6" required
                            class="input-zocco otp-input w-full py-5 rounded-xl text-white font-bold tracking-[0.8em]">
                    </div>
                </div>

                <button type="submit"
                    class="bg-yellow-zocco hover:bg-yellow-700 text-white w-full py-4 rounded-xl font-bold text-xs uppercase tracking-[0.2em] transition-all transform active:scale-95 shadow-xl shadow-yellow-900/20 mt-4">
                    Konfirmasi Kode
                </button>
            </form>

            <div class="mt-8">
                <div class="w-full border-t border-white/10 mb-6"></div>
                <p class="text-center text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                    Tidak menerima kode?
                    <a href="#" class="text-yellow-zocco hover:underline decoration-2 underline-offset-4">Kirim
                        Ulang</a>
                </p>
            </div>

            <p class="text-center mt-6 text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                <a href="{{ route('register') }}"
                    class="hover:text-white transition flex items-center justify-center gap-2">
